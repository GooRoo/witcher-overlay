...
Text::Text(Renderer *renderer, const std::string& font,int iFontSize,bool Bold,bool Italic,int x,int y,D3DCOLOR color,const std::string& text, bool bShadow, int align, bool bShow)
...
bool Text::drawText(int x, int y, DWORD dwColor, const std::string& strText, DWORD dwFlags /*= 0L*/)
{
	...
	{
	case Center:
		dwFlags |= D3DFONT_CENTERED_X;
		dwFlags |= D3DFONT_CENTERED_Y;
		break;

	case BottomRight:
		dwFlags |= D3DFONT_ALIGNRIGHT;
		dwFlags |= D3DFONT_ALIGNBOTTOM;
		break;
