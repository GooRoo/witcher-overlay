...
void TextCreate(Serializer& serializerIn, Serializer& serializerOut)
{
	...
	WRITE(g_pRenderer.add(std::make_shared<Text>(&g_pRenderer, Font, FontSize, bBold, bItalic, x, y, color, string, bShadow, 0, bShow)));
}
...
void SetOverlayAssetsPath(Serializer& serializerIn, Serializer& serializerOut)
{
	READ(std::string, path);

	g_Overlay.setAssetsPath(path);
}